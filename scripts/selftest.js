...
const checks=[];
checks.push(unwrap(await post('initialize',{protocolVersion:'2025-06-18',capabilities:{},clientInfo:{name:'selftest',version:'3.0.0'}})));
...
const chat=unwrap(await post('tools/call',{name:'ingest_chat_transcript',arguments:{title:'chat test',text:'User: hello\nAssistant: hi'}})); checks.push(chat);
const att=unwrap(await post('tools/call',{name:'ingest_attachment_base64',arguments:{filename:'note.txt',mime:'text/plain',base64:Buffer.from('attachment text').toString('base64')}})); checks.push(att);
const bundle=unwrap(await post('tools/call',{name:'create_fable_bundle',arguments:{title:'Selftest bundle',question:'Review these resources.',resourceIds:[rid,chat.structuredContent.id,att.structuredContent.id,img.structuredContent.id],includeFullText:true,maxBytesPerFile:5000}})); checks.push(bundle);
...
const ok=checks.length===23;
console.log(JSON.stringify({ok,count:checks.length,registered:rid,jobId:job.structuredContent.jobId,imageId:img.structuredContent.id,bundlePath:bundle.structuredContent.bundlePath},null,2));
